...
} from '../../../../../server/services/email/productEmailTemplates.ts';
...
test('buildOrderEmail formats order notifications and escapes HTML fields', () => {
  ...
  assert.match(merchantEmail.text, /Open order desk\nhttps:\/\/example\.com\/orders\/order-123/);
  assert.doesNotMatch(merchantEmail.text, /^Action:/m);
  assert.doesNotMatch(merchantEmail.text, /^Open order desk:/m);
...
test('product emails use branded user-facing copy without scenario enum fields', () => {
  ...
  assert.match(orderEmail.text, /pending merchant review/i);
  assert.match(orderEmail.html, /4real/i);
  assert.doesNotMatch(orderEmail.text, /Scenario:/);
  assert.doesNotMatch(orderEmail.text, /order_created_user/);
  assert.doesNotMatch(orderEmail.html, /Scenario/);

  const failedWithdrawalEmail = buildWithdrawalEmail({
  ...
  assert.match(failedWithdrawalEmail.text, /held balance was refunded/i);
  assert.doesNotMatch(failedWithdrawalEmail.text, /provider rejected request/i);
  assert.doesNotMatch(failedWithdrawalEmail.html, /provider rejected request/i);
  assert.doesNotMatch(failedWithdrawalEmail.text, /Last error:/);
});

test('product emails render action links as CTA copy, not detail fields', () => {
  const depositEmail = buildDepositEmail({
    ...
    memo: '<missing>',
    username: 'bob',
    actionUrl: 'https://example.com/merchant/deposits',
  });

  assert.match(depositEmail.text, /Review deposits\nhttps:\/\/example\.com\/merchant\/deposits/);
  assert.doesNotMatch(depositEmail.text, /^Action:/m);
  assert.doesNotMatch(depositEmail.text, /^Review deposits:/m);
