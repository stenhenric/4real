...
function buildStatusLabel(dashboard: MerchantDashboardDTO | null) {
  if (!dashboard) {
    return {
...
export function MerchantLayout() {
  ...
  const statusBadge = buildStatusLabel(dashboard);
              ...
              <div>
                <p className="text-[11px] font-mono font-bold uppercase tracking-[0.35em] opacity-50">
                  {location.pathname === '/merchant'
                    ? 'Overview'
                    : location.pathname.startsWith('/merchant/orders')
                      ? 'Order Desk'
                      : location.pathname.startsWith('/merchant/liquidity')
                        ? 'Liquidity'
                        : 'Alerts'}
                </p>
