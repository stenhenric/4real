...
interface MerchantBadgeCounts {
  alertBadge: number;
  pendingBadge: number;
  unmatchedDepositBadge: number;
}

function getNavBadge(to: typeof NAV_ITEMS[number]['to'], badges: MerchantBadgeCounts) {
  if (to === '/merchant/orders') {
    return badges.pendingBadge;
  }

  if (to === '/merchant/deposits') {
    return badges.unmatchedDepositBadge;
  }

  if (to === '/merchant/alerts') {
    return badges.alertBadge;
  }

  return null;
}

function MerchantNavLinkList({
  badges,
  mobile = false,
}: {
  badges: MerchantBadgeCounts;
  mobile?: boolean;
}) {
  return (
    <>
      {NAV_ITEMS.map((item) => {
        const badge = getNavBadge(item.to, badges);
...
function MerchantReserveSummary({
  dashboard,
  pendingBadge,
  ...
  return (
    <aside className="hidden w-72 shrink-0 flex-col gap-4 lg:flex">
      ...
      <nav className="rough-border bg-white/85 p-3 shadow-lg">
        <MerchantNavLinkList badges={badges} />
      </nav>

      <MerchantReserveSummary
        dashboard={dashboard}
        pendingBadge={badges.pendingBadge}
        unmatchedDepositBadge={badges.unmatchedDepositBadge}
      />
    </aside>
  );
}
      ...
      <div className="mt-4 space-y-3 lg:hidden">
        <nav aria-label="Merchant sections" className="-mx-1 overflow-x-auto px-1 pb-1">
          <div className="flex w-max gap-2">
            <MerchantNavLinkList badges={badges} mobile />
          </div>
        </nav>
  ...
  return (
    <div className="min-h-screen bg-paper paper-texture">
      <div className="mx-auto flex min-h-screen max-w-[1400px] gap-6 px-4 py-6 lg:px-6">
        <MerchantSidebar badges={badges} dashboard={dashboard} />
